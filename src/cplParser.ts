...
import { INTEROP_CPL_URI, SMPTE_CPL_URI } from './util';
import { dcpType } from './enums';

export default function cplParser(
	cplXMLContent: string,
				...
				for (const key in assetList) {
					...
					if (Array.isArray(assetList[key])) {
						assetList[key].forEach((as: any) => {
							...
							if (
								cplObject.assetList &&
								cplObject.assetList.findIndex((as) => as.id === asset.id) >= 0
							)
	...
	if (cplRawObject['@_xmlns'] == INTEROP_CPL_URI) {
		cplObject.type = dcpType.INTEROP;
	} else if (cplRawObject['@_xmlns'] == SMPTE_CPL_URI) {
		cplObject.type = dcpType.SMPTE;
	} else {
		cplObject.type = '';
	}
